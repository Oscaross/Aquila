...
public class Star : MonoBehaviour
{
    ...
    public void Init(float framesPerSecond, float holdTime)
    {
        ...
        dayNightCycleOffset = Random.Range(-0.06f, 0.06f);

        SetFrame(0);
    }

    public void UpdateStar(float t)
    {
        sr.color = new Color(sr.color.r, sr.color.g, sr.color.b, SkyController.Instance.GetCurrentLightSourceLevel(t + dayNightCycleOffset));
        if (rate <= 0f) return; // static (no twinkle) so our job here is done

        float local = Mathf.Repeat(t + offset, cycleTime); // how many seconds this star is into its animation loop
